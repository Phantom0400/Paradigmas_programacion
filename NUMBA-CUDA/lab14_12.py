...
# Tamanyo del dominio (menor que uno)
...
@cuda.jit
def solucion_kernel(u_d, un_d, udx2_0, udx_1, dt, n_0, n_1, kd):
    ii, jj = cuda.grid(2)
    i = ii + n_0*jj
    if ii == 0 or jj == 0 or ii == n_0 - 1 or jj == n_1 - 1:
        unueva = 0.0
    else:
        unueva = evolucion_gpu(un_d, udx2_0, udx_1, dt, n_0, n_1, kd, i)
    if i == int((n_0*n_1)/2) + int(n_0/2):
        unueva = 1.0
    un_d[i] = unueva
blockdim = (32, 16) # Hilos por bloque
griddim = (int(n[0]/blockdim[0]), int(n[1]/blockdim[1])) # Cloques en el dominio
""" Programa Principal """
start = time.time()
u = np.zeros(nt, dtype = np.float64) # Arreglo de lectura
un = np.zeros(nt, dtype = np.float64) # Arreglo de escritura
# Pasar los arreglos al GPU
...
for t in range(1, pasos + 1):
    ...
    if t%100 == 0: print('Paso = ' + str(t))
#Pasar arreglo al GPU
u_d.copy_to_host(u)
end = time.time()
print('Tardo: ' + str(end - start) + 's')
# Graficar en 3d
